...
import type { StageId } from './StageControls';
import { PIPELINE_STAGES } from './StageControls';
...
function isStageComplete(stageId: StageId, stopAtStage: StageId | null, disabledStages: StageId[]): boolean {
...
export default function ResultPanels({ result, demoStopInfo }: ResultPanelsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Affected Services */}
      {!impactStop.show && hasFailureData && result.affectedCallers && result.affectedCallers.length > 0 && (
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {result.affectedCallers.map((caller: { serviceId?: string; id?: string; impact?: string; reason?: string; description?: string }) => (
                  <tr key={caller.serviceId ?? caller.id ?? crypto.randomUUID()} className="border-b border-slate-800 hover:bg-slate-800/50">
      ...
      {!pathsStop.show && hasFailureData && result.criticalPathsToTarget && result.criticalPathsToTarget.length > 0 && (
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-6">
          <h2 className="text-xl font-semibold text-white mb-4">
            Critical Paths ({result.criticalPathsToTarget.length})
          </h2>
          <div className="space-y-3">
            {result.criticalPathsToTarget.map((path: { id?: string; length?: number }, pathIndex: number) => (
              <div key={path.id ?? `path-${pathIndex}`} className="bg-slate-800 border border-slate-600 rounded p-4">
      ...
      {!pathsStop.show && hasScaleData && result.affectedPaths && result.affectedPaths.length > 0 && (
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-6">
          <h2 className="text-xl font-semibold text-white mb-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {result.affectedPaths.map((path: { path?: string; id?: string; metric?: string; change?: string; impact?: string }, pathIndex: number) => (
                  <tr key={path.id ?? path.path ?? `affected-path-${pathIndex}`} className="border-b border-slate-800 hover:bg-slate-800/50">
      ...
      {!recsStop.show && result.recommendations && result.recommendations.length > 0 && (
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-6">
          ...
          <ul className="space-y-2">
            {result.recommendations.map((rec: string | { id?: string; text?: string }, recIndex: number) => {
              const recKey = typeof rec === 'string' ? `rec-${recIndex}` : (rec.id ?? `rec-${recIndex}`);
