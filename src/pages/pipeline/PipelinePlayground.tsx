...
import ResultPanels, { type DemoStopInfo } from './components/ResultPanels';
import StageControls, { PIPELINE_STAGES, type StageId, type StageState, type StageStatus } from './components/StageControls';
...
export default function PipelinePlayground() {
  ...
  const stageStatesMap = stageStates.reduce<Record<StageId, StageState>>((acc, s) => {
    acc[s.id] = s;
    return acc;
  }, {} as Record<StageId, StageState>);

  // Stop at index calculation
  const stopAtIndex = stopAtStage 
    ? PIPELINE_STAGES.findIndex(s => s.id === stopAtStage)
    : null;
          ...
          {!result && !loading && !error && (
            <div className="bg-slate-900 border border-slate-700 rounded-lg p-8 text-center">
              ...
              <p className="text-slate-400">
                Configure a scenario and click Run to see results
              </p>
            </div>
          )}
