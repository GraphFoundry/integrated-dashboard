...
export function NodeDetailsDrawer({
  ...
  edges,
  onClose,
}: Readonly<NodeDetailsDrawerProps>) {
  ...
  return (
    <div className="absolute top-4 right-4 z-20 w-96 bg-slate-900/95 backdrop-blur border border-slate-700 rounded-lg shadow-xl max-h-[calc(100%-2rem)] overflow-hidden flex flex-col animate-in fade-in slide-in-from-right duration-200">
      ...
      <div className="flex border-b border-slate-700 bg-slate-800/30">
        ...
        <Tab
          label="Blast Radius"
          active={activeTab === 'blastRadius'}
          onClick={() => setActiveTab('blastRadius')}
        />
        <Tab
          label="Suspects"
          active={activeTab === 'suspects'}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {activeTab === 'blastRadius' && <BlastRadiusTab blastRadius={blastRadius} nodes={nodes} />}

        {activeTab === 'suspects' && (
          <SuspectsTab suspects={suspects} showAdvanced={showAdvanced} />
        )}

        {activeTab === 'flows' && <FlowsTab flows={flows} showAdvanced={showAdvanced} />}
      </div>
    </div>
  )
}
  ...
  return (
    <>
      ...
      <div>
        ...
        <div
          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${getRiskBadgeClass(node.riskLevel)}`}
        >
          {node.riskLevel}
        </div>
      </div>
        ...
        {suspects.map((suspect, idx) => (
          <div key={suspect.nodeId} className="p-3 bg-slate-800/30 rounded-lg">
            <div className="flex items-start justify-between mb-2">
              ...
              <div
                className={`text-xs px-2 py-0.5 rounded ${getRiskBadgeClass(suspect.riskLevel)}`}
              >
                {suspect.riskLevel}
              </div>
            </div>
